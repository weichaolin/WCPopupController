...
class WCPopupController: NSObject, UIGestureRecognizerDelegate{
    ...
    var maskTouched: ((WCPopupController) -> Void)?
    var willPresent: ((WCPopupController) -> Void)?
    var didPresent: ((WCPopupController) -> Void)?
    var willDismiss: ((WCPopupController) -> Void)?
    var didDismiss: ((WCPopupController) -> Void)?
    ...
    private var maskAlpha: CGFloat = 0.5
    ...
    private var maskType: WCPopupMaskType = .BlackTranslucent
    ...
    init(maskType: WCPopupMaskType) {
        super.init()
        setupSubviews(maskType: maskType)
    }
    
    private func setupSubviews(maskType: WCPopupMaskType) {
        // superview
   ...
   @objc private func handleTap() {
    ...
    private func _dismiss(duration: TimeInterval, isSpringAnimated: Bool) {
        ...
        let animOpts: (WCPopupSlideStyle) -> UIView.AnimationOptions = { slide in
...
extension WCPopupController {
    func popupController(maskType: WCPopupMaskType) -> WCPopupController {
        return WCPopupController(maskType: maskType)
    }
...
extension UIViewController {
    ...
    var wc_popupController: WCPopupController {
        set {
            objc_setAssociatedObject(self, &AssociatedKey.key, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
        get {
            var popupController = objc_getAssociatedObject(self, &AssociatedKey.key) as? WCPopupController
            if popupController == nil {
                popupController = WCPopupController()
                self.wc_popupController = popupController!
            }
